...
export function DashboardSidebar({ open, onClose }: DashboardSidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 z-40 h-screen w-[280px] border-r border-teal-300/30 bg-gradient-to-b from-[#0f8f96] via-[#076176] to-[#03384d] text-slate-100 shadow-2xl transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm font-bold uppercase tracking-[0.03em] transition ${active
