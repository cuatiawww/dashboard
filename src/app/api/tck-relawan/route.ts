...
const VERIFIED_NTT_TCK_LIST: any[] = [
  {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Format FormData (sama persis dengan PHP cURL array postfields)
    ...
    try {
      ...
      console.warn('[TCK Kemkes Fetch Warning]:', e.message)
    }

    // Jika live API Kemkes diblokir Cloudflare (HTTP 403 Forbidden) / offline, fallback ke data terverifikasi NTT
    if (!isLiveSuccess || relawanList.length === 0) {
      if (targetProp === '53' || targetProp.toLowerCase().includes('ntt') || targetProp.toLowerCase().includes('nusa tenggara timur')) {
        relawanList = kd_kab
          ? VERIFIED_NTT_TCK_LIST.filter((r) => String(r.kd_kab) === String(kd_kab) || String(r.kab_kota).toLowerCase().includes(String(kd_kab).toLowerCase()))
          : VERIFIED_NTT_TCK_LIST
        isFallback = true
      }
    }
    ...
    return NextResponse.json({
      success: true,
      status: true,
      source: isLiveSuccess ? 'live_kemkes_api' : 'verified_ntt_tck_dataset',
    ...
    return NextResponse.json({
      success: true,
      status: true,
      source: 'verified_ntt_tck_dataset',
      total: VERIFIED_NTT_TCK_LIST.length,
      data: VERIFIED_NTT_TCK_LIST,
      filter: { kd_prop: '53' }
    })
  }
}
